...
export default function NonAssetsPage({ items, setItems, addHistory }) {
  const [q, setQ] = useState("");
  const [editing, setEditing] = useState(null);
  const [showForm, setShowForm] = useState(false);

  const filtered = items.filter(i => i.type === "non-asset" && (i.name.toLowerCase().includes(q.toLowerCase()) || i.id.toLowerCase().includes(q.toLowerCase())));
    ...
    <div className="container page">
      <div className="page-head">
        ...
        <div className="actions">
          ...
          <button className="btn" onClick={() => { setShowForm(true); setEditing(null); }}>Tambah Non-Asset</button>
        </div>
      </div>
